...
function fixture(blank=true){const origin={id:'original',cwd:'/owned',blank,projectionValues:{agentPreset:'standard'}};
 const list={current:'original',byId:{original:origin}},calls=[];
 const state={current:'standard',busy:false,options:['standard','ptc','minimal','cordis'].map(id=>({id}))};
 const seat={store:{getSnapshot:()=>state},set:patch=>Object.assign(state,patch),stage(id){this.staged=id;state.current=id;}};
...
test('还没有会话时只暂存下一会话选择，不报告一次不存在的 Host 切换',async()=>{
 const f=fixture();f.list.current=undefined;assert.equal(await f.run('cordis'),undefined);assert.equal(f.seat.staged,'cordis');assert.equal(f.calls.length,0);
});
