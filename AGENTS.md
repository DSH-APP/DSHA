...
| `pnpm-env-fix.sh` | before every `pnpm install` (source route + plugin install) | Turn off pnpm's hard-link import **where pnpm ≥10 actually reads it** (`pnpm-workspace.yaml`, not `.npmrc` — see traps), clear failed git-hosted staging dirs, drop dangling `.l2s` links in the store |
...
- **`flock(2)` works on app-private storage but not on the public FUSE volume.** Measured on device (V2352A, Android 16, node 24.19, self-built `fs-ext`): `/root/…` under app-private storage (ext4) and `/tmp` both take the lock, exclusion holds across descriptors, and the inode check passes; `/sdcard/Documents/dshdata`, `/sdcard/Download/DSHA` and every other primary-external path return **`ENOSYS`**. proot/proroot itself is not the obstacle — the private-directory rows prove that. This decides where session data may live: dsh 0.1.3 puts a `session.lock` in every session directory and its lease only treats `EAGAIN`/`EWOULDBLOCK` as contention, so any other errno propagates and **opening a session fails outright**. Hence `session-home.sh` keeps `.dsh/sessions` as a real directory in private storage (mirroring to the public volume for reinstall recovery) while `storages`/`attachments`/`settings.yaml` stay symlinked to the public volume through `migrate-public-data.sh`, and `session-lock-patch.sh` is the second line of defence for paths that slip through (migration in flight, restore from mirror, hand-edited config, other ROMs). Assertions: `tools/session-home-test.sh` (24) and `tools/session-lock-patch-test.sh` (12) — the latter deliberately checks both directions, since a patch that swallows `EAGAIN` too would let two writers into one log.

`link()` under `/data/user/0/<pkg>/` fails with `AccessDeniedException` (SELinux), which is why proot needs `--link2symlink` at all. The extension emulates a link as *symlink → `.l2s.` intermediate*, stored next to the source unless `PROOT_L2S_DIR` is set. dsh publishes a **new** file with `link(temp, target)` and then deletes its staging dir — so every freshly written file became a dangling symlink (`write` reported success, the file was unreadable, `edit` was fine because it uses `rename`). Fixed by `fs-write-patch.sh` (publish new files with `rename`) plus `PROOT_L2S_DIR=<rootfs>/.l2s` as a fallback. Don't "simplify" this away.

- **pnpm ≥10 的这类设置只从 `pnpm-workspace.yaml` 读，写进 `.npmrc` 等于没写.** 对照实验（2026-08-30，pnpm 11.21.0，同一个包同一个 store）：`.npmrc` 里 `package-import-method=copy` → pnpm 照样打印 *Packages are hard linked*，`stat` 出来 nlink=5；`pnpm-workspace.yaml` 里 `packageImportMethod: copy` → *Packages are copied*，nlink=1。dsh 自己的 profile 模板注释也写着「pnpm ≥10 reads its settings from pnpm-workspace.yaml, not .npmrc」。所以「关掉硬链接导入」这条修复曾长期是死代码 —— link 被 `--link2symlink` 模拟的机器上 pnpm 一直在铺 `.l2s` 链，悬空后表现为莫名 ENOENT，dsh 侧则因为 `require.resolve` 把 realpath 解进 store 的扁平哈希目录而报 `MissingClientBundleError`。现在由 `pnpm-env-fix.sh` §③ 往 profile 与工作区的 `pnpm-workspace.yaml` 幂等追加，`.npmrc` 那两行只留给 pnpm 9 / npm。**写之前先探测**（容器内 `ln` 一次看结果是不是 symlink，`DSHA_LINK_EMULATED=1/0` 可覆盖）：真硬链接可用的机器强行改成复制会白扔掉 store 去重，profile 装几十个插件能多占几百 MB。断言在 `tools/pnpm-import-test.sh`（沙箱里真跑脚本、真查文件，18 条）。
