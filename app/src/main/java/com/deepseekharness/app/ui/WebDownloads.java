...
public final class WebDownloads {
    ...
    public WebDownloads(AppCompatActivity activity, Bundle saved) {
        ...
        model.state.observe(activity, state -> {
            ...
            if (working) {
                if (dialog == null) dialog = new AlertDialog.Builder(activity).setTitle("保存网页文件")
                        .setMessage("").setNegativeButton("取消", (d,w) -> model.cancel()).setCancelable(false).create();
                ...
                dialog.setMessage(state.message() + count); dialog.show();
            } else {
                dismiss();
                if (state.phase().equals("ready") && !model.pickerOpen) {
                    ...
                    if (state.phase().equals("error") && model.canRetrySave()) {
                        dialog = new AlertDialog.Builder(activity).setTitle("文件尚未保存").setMessage(state.message())
                                .setPositiveButton("更换位置重试",(d,w) -> model.retrySave())
                                .setNegativeButton("取消",(d,w) -> model.acknowledge()).setCancelable(false).create(); dialog.show();
