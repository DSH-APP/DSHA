...
public final class StorageActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle saved){super.onCreate(saved);
        page=new CardPage(this,t("存储空间","Storage"),t("完整 Linux 环境约占 1 GB。历史环境和回退副本另行保留；它们不等于聊天数据。以下为文件大小估算。","The full Linux environment uses about 1 GB. Older environments and rollback copies are retained separately from chats. Sizes below are estimates."));UiNavigation.addHeader(this,page.root,t("存储空间","Storage"),this::finish);
        ...
        clean=page.button(page.footer,t("清理可再生缓存与多余运行时副本","Clean reproducible caches and excess runtime copies"),true,()->new DshaDialogBuilder(this).setTitle(t("清理存储空间","Clean storage"))
                .setMessage(t("请先停止 DSH 和终端。清理历史验收缓存，以及超过保留数量且已核验的受管回退副本。对话、附件、设置、插件、手动备份和无法核验的原件保留。","Stop DSH and terminals first. Removes old verification caches and verified excess managed rollback copies. Keeps chats, attachments, settings, plugins, manual backups and unverified originals."))
    ...
    private void load(boolean remove){if(busy)return;busy=true;clean.setEnabled(false);refresh.setEnabled(false);state.setText(t("正在处理…","Working…"));
        new Thread(()->{try{long reclaimed=remove?StorageMaintenance.clean(this):0;var sizes=StorageMaintenance.inspect(this);runOnUiThread(()->{
            ...
            state.setText(remove?t("已释放 ","Freed ")+com.deepseekharness.app.util.Fmt.bytes(reclaimed):t("已完成统计。没有使用恢复功能时，旧环境保护副本仍可能占用空间，可在保留数据页面查看。","Sizes updated. Environment protection copies can remain even without restoring. Inspect them under Retained data."));done();
        });}catch(Exception error){runOnUiThread(()->{state.setText(com.deepseekharness.app.backup.NativeBackupJobs.code(error));done();});}},"storage-maintenance").start();
    }
